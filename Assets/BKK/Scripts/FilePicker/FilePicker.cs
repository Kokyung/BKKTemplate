...
namespace BKK.Native
{
    ...
    public class FilePicker : MonoBehaviour
    {
        ...
        private string[] documentFileTypes = {"pdf"};
        private string[] videoFileTypes = {"mp4", "avi", "mkv", "mov"};
        private string[] audioFileTypes = {"wav", "mp3"};
        private string[] imageFileTypes = {"jpeg", "jpg", "png"};
...
#if UNITY_EDITOR
            if (GUI.Button(new Rect(Screen.width - 150, 50, 100, 40), "Pick PDF File"))
            {
                PickDocumentFile(delegate(string[] pathList)
                {
                    var pdfPath = pathList.First(t => t.EndsWith("pdf", StringComparison.OrdinalIgnoreCase));
                    LoadPdf(pdfPath);
                });
            }

            if (GUI.Button(new Rect(Screen.width - 150, 100, 100, 40), "Pick Video File"))
            {
                PickVideoFile(delegate(string[] pathList)
                {
                    var videoPath = pathList.First();
                    LoadVideo(videoPath);
                });
            }
#endif
        ...
        public void PickDocumentFile(Action<string[]> afterAction)
        {
            PickFile(documentFileTypes, afterAction);
        }
        ...
        {
            PickFile(videoFileTypes, afterAction);
        }
        ...
        /// </summary>
        /// <param name="afterAction"></param>
        public void PickAudioFile(Action<string[]> afterAction)
        {
            PickFile(audioFileTypes, afterAction);
        }
        ...
        /// </summary>
        /// <param name="afterAction"></param>
        public void PickImageFile(Action<string[]> afterAction)
        {
            PickFile(imageFileTypes, afterAction);
        }
        ...
        private void PickPath(bool multipleSelect = false)
        {
            completed = false;
        ...
        private void ConvertExtensionToFileTypeList(string[] fileTypes)
        {
            fileMIMEList.Clear();
