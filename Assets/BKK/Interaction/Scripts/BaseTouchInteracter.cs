...
public class BaseTouchInteracter : MonoBehaviour
{
    ...
    protected static Vector3 pointPos;
    ...
    {
        BKKInputSystem.inputs.Touch.SingleTap.started += OnClickStarted;
        BKKInputSystem.inputs.Touch.SingleTap.canceled += OnClickCanceled;
        BKKInputSystem.inputs.Touch.SingleTap.performed += OnClickPerformed;
    }

    protected void FindCamera()
    {
        StartCoroutine(Co_FindCamera());
    }

    private IEnumerator Co_FindCamera()
    {
    ...
    protected virtual void OnClickCanceled(InputAction.CallbackContext callback)
    {
    }
    ...
    {
        if (!BKKInputSystem.Initialized) return;

        pointPos = BKKInputSystem.inputs.Touch.Point.ReadValue<Vector2>();
    }
    ...
    public RaycastHit GetWorldPositionHitData(Vector3 screenPosition)
    {
        ...
        if (BKKInputSystem.inputs.Touch.SecondaryTouch.triggered) return emptyHit;
